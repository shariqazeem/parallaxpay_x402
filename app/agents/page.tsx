  ...
  const [agents, setAgents] = useState<AgentStats[]>([
    {
      ...
      avgCost: 0.00118,
      successRate: 98.4,
      lastAction: 'Found 8.3% arbitrage opportunity',
    ...
    },
    {
      ...
      avgCost: 0.00109,
      successRate: 99.1,
      lastAction: 'Executing trade via ParallaxNode-Epsilon',
    ...
    },
    {
      ...
      avgCost: 0.00125,
      successRate: 97.8,
      lastAction: 'Bulk purchased 5000 tokens',
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const actions = [
        ...
        'Switched to cheaper provider',
        'Bulk purchased tokens',
        'Optimized routing through EU-West',
        'Completed x402 payment',
      ...
      setAgents((prev) =>
        prev.map((a, i) =>
          i === agentIndex
            ? {
                ...a,
                status: Math.random() > 0.6 ? 'executing' : 'active',
                lastAction: actions[Math.floor(Math.random() * actions.length)],
                lastActionTime: Date.now(),
                totalTrades: a.totalTrades + (Math.random() > 0.7 ? 1 : 0),
                profit: a.profit + Math.random() * 2,
              }
            : a
        )
      )
      ...
      if (Math.random() > 0.5) {
        const newTrade: Trade = {
          ...
          provider: `ParallaxNode-${['Alpha', 'Beta', 'Gamma', 'Delta'][Math.floor(Math.random() * 4)]}`,
        ...
        setTotalVolume((prev) => prev + newTrade.cost)
      }
    }, 3000)
        ...
        <div className="max-w-[1920px] mx-auto px-6 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button className="glass-hover neon-border px-6 py-2 rounded-lg text-sm font-semibold hover:scale-105 transition-all">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <StatCard label="Active Agents" value={agents.filter((a) => a.status !== 'idle').length} icon="🤖" />
